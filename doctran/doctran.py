...
class Document(BaseModel):
    ...
    def extract(
        self, *, properties: List[ExtractProperty]
    ...
    def redact(
        ...
        *,
        ...
        transformation_builder = DocumentTransformationBuilder(self)
        transformation_builder.redact(
            entities=entities, spacy_model=spacy_model, interactive=interactive
        )
        return transformation_builder
...
class DocumentTransformationBuilder:
    ...
    def execute(self) -> Document:
        ...
        transformed_document = self.document.model_copy()
        transformation = None
        try:
            for transformation in self.transformations:
                transformer = getattr(module, transformation[0].value)(
                    config=transformed_document.config, **transformation[1]
                )
    ...
    def extract(
        ...
        self.transformations.append(
            (Transformation.extract, {"properties": properties})
        )
        return self

    def summarize(self, token_limit: int = 100) -> "DocumentTransformationBuilder":
        self.transformations.append(
            (Transformation.summarize, {"token_limit": token_limit})
        )
        return self
    ...
    def process_template(
        ...
        self.transformations.append(
            (Transformation.process_template, {"template_regex": template_regex})
        )
        return self
